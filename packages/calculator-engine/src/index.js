...
exports.calculateMinDimension = calculateMinDimension;
exports.calculateMaxDimension = calculateMaxDimension;
exports.calculateFabricMeters = calculateFabricMeters;
exports.calculateSewingCost = calculateSewingCost;
exports.calculateFiberfillCost = calculateFiberfillCost;
exports.calculatePipingCost = calculatePipingCost;
exports.calculateTiesCost = calculateTiesCost;
exports.getFabricTier = getFabricTier;
exports.calculateFabricCost = calculateFabricCost;
 ...
 */
function calculateMinDimension(shape, dims) {
    switch (shape) {
        case 'Rectangle':
 ...
 */
function calculateMaxDimension(shape, dims) {
    switch (shape) {
        case 'Rectangle':
 ...
 */
function calculateFabricMeters(shape, dims) {
...
// ==================== COST CALCULATIONS ====================
function calculateSewingCost(maxDimension, quantity) {
    if (maxDimension >= 0 && maxDimension <= 24)
        return 7.5 * quantity;
    if (maxDimension >= 25 && maxDimension <= 48)
        return 11.25 * quantity;
    if (maxDimension >= 49 && maxDimension <= 72)
        return 15 * quantity;
    if (maxDimension >= 73 && maxDimension <= 96)
        return 18.75 * quantity;
    if (maxDimension >= 97 && maxDimension <= 120)
        return 21.25 * quantity;
    return 0;
}
function calculateFiberfillCost(foamType, maxDimension, minDimension, thickness, quantity, shape, dims) {
    if (foamType === 'Covers Only')
        return 0;
    const F7 = dims.length || 0;
    const F8 = dims.width || 0;
    const F32 = dims.bottomWidth || 0;
    ...
    if (foamType === 'High Density Foam') {
        if (maxDimension >= 0 && maxDimension <= 12)
            return 0.06237 * F88 * F10 * F82;
        if (maxDimension >= 13 && maxDimension <= 18)
            return 0.093555 * F88 * F10 * F82;
        if (maxDimension >= 19 && maxDimension <= 36)
            return 0.176715 * F88 * F10 * F82;
        if (maxDimension >= 37 && maxDimension <= 48)
            return 0.259875 * F88 * F10 * F82;
        if (maxDimension >= 49 && maxDimension <= 60)
            return 0.3170475 * F88 * F10 * F82;
        if (maxDimension >= 61 && maxDimension <= 72)
            return 0.384615 * F88 * F10 * F82;
        if (maxDimension >= 73 && maxDimension <= 84)
            return 0.4521825 * F88 * F10 * F82;
        if (maxDimension >= 85 && maxDimension <= 96)
            return 0.51975 * F88 * F10 * F82;
        if (maxDimension >= 97 && maxDimension <= 108)
            return 0.5873175 * F88 * F10 * F82;
        if (maxDimension >= 109 && maxDimension <= 120)
            return 0.654885 * F88 * F10 * F82;
        return 0;
    }
    if (foamType === 'Dry Fast Foam') {
        if (maxDimension >= 0 && maxDimension <= 12)
            return 0.081081 * F88 * F10 * F82;
        if (maxDimension >= 13 && maxDimension <= 18)
            return 0.1216215 * F88 * F10 * F82;
        if (maxDimension >= 19 && maxDimension <= 36)
            return 0.2297295 * F88 * F10 * F82;
        if (maxDimension >= 37 && maxDimension <= 48)
            return 0.3378375 * F88 * F10 * F82;
        if (maxDimension >= 49 && maxDimension <= 60)
            return 0.41216175 * F88 * F10 * F82;
        if (maxDimension >= 61 && maxDimension <= 72)
            return 0.4999995 * F88 * F10 * F82;
        if (maxDimension >= 73 && maxDimension <= 84)
            return 0.58783725 * F88 * F10 * F82;
        if (maxDimension >= 85 && maxDimension <= 96)
            return 0.675675 * F88 * F10 * F82;
        if (maxDimension >= 97 && maxDimension <= 108)
            return 0.76351275 * F88 * F10 * F82;
        if (maxDimension >= 109 && maxDimension <= 120)
            return 0.8513505 * F88 * F10 * F82;
        return 0;
    }
...
function calculatePipingCost(piping, maxDimension, quantity) {
    if (piping === 'No Piping' || maxDimension === 0)
        return 0;
    if (piping === 'Piping' && maxDimension >= 1 && maxDimension <= 24)
        return 3 * quantity;
    if (piping === 'Piping' && maxDimension >= 25 && maxDimension <= 48)
        return 6 * quantity;
    if (piping === 'Piping' && maxDimension >= 49 && maxDimension <= 72)
        return 7.5 * quantity;
    if (piping === 'Piping' && maxDimension >= 73 && maxDimension <= 96)
        return 9 * quantity;
    if (piping === 'Piping' && maxDimension >= 97 && maxDimension <= 120)
        return 10 * quantity;
    return 10 * quantity;
}
function calculateTiesCost(ties, quantity) {
    if (ties === 'No ties')
        return 0;
    if (ties === '2 Side')
        return 20 * quantity;
    if (ties === '4 Side' || ties === '4 Corner')
        return 30 * quantity;
    if (ties === '6 Side')
        return 30 * quantity;
    if (ties === '8 Side')
 ...
 */
function getFabricTier(fabricCode) {
    // Check exact match
...
function calculateFabricCost(fabricCode, fabricMeters, quantity, fabricPrice) {
    if (!fabricCode)
        return 0;
...
function calculateQuote(selections) {
    ...
    const fiberfillCostUnit = roundTo(calculateFiberfillCost(selections.foamType, maxDimension, minDimension, dims.thickness || 0, 1, selections.shape, dims), 4);
    ...
    const fabricCostUnit = roundTo(calculateFabricCost(selections.fabricCode, fabricMeters, 1, selections.fabricPrice), 4);
 ...
 */
function calculateFinalPrice(selections, markupType, markupValue) {
